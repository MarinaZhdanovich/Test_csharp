...
{
    int[] result = new int[max - min + 1]; // + 1 чтобы было включительно
    // 0   1      2 3 4 5 6 7
    //min min + 1...        max - частота встречаемости
  // Например: max - min + 1 = (7 -(-5) + 1 = будет иметь 13 элементов для хранения частоты встречаемости каждого значения от -5 до 7

    for (int i = 0; i < array.GetLength(0); i++)
    {
        for (int j = 0; j < array.GetLength(1); j++)
        {
            int index = array[i, j] - min; // min = -5 узнаем индекс элемента в массиве result                
            result[index]++;     // отнимаем min, потому что мы хотим, чтобы самое маленькое значение min соответствовало индексу 0 ( -5 - (-5) = 0) array[i, j] = -4, index = -4 -(-5) = -4 + 5 = 1                   
        }                                       
    }      // сколько раз встретили значение
    return result;
}
void Result(int[] dict, int min)
{
    ...
    {
        Console.WriteLine($"Число {i + min} встречается {dict[i]} раз(а)");   // i + min = чтобы получить исходное число. 
    }
}
