...
void ChangeRowColumns(int[,] array)
{
    ...
    else
    {
        ...
        for (int i = 0; i < array.GetLength(0); i++)
        {
            for (int j = i; j < array.GetLength(1); j++)   //j = i позволяет избежать повторной обработки мы находимся на диагонали, ее не меняем
               {
                temp = array[j, i];
                array[j, i] = array[i, j];
                array[i, j] = temp;
...
/*
Диагональ не меняем i = j
Меняем: array[0, 1] и array[1, 0]
Меняем: array[0, 2] и array[2, 0]
Меняем: array[1, 2] и array[2, 1]
*/
